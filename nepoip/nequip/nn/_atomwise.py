...
from e3nn.o3 import Linear, FullyConnectedTensorProduct, Irreps
...
class AtomwiseLinear_pol(GraphModuleMixin, torch.nn.Module):
    def __init__(
        ...
        esp_repeat = 1
        self.esp_repeat = esp_repeat
        emb_irreps_out = Irreps([(self.irreps_out[out_field].dim-esp_repeat, (self.irreps_out[out_field].lmax, 1))])

        self.linear = Linear(
            irreps_in=self.irreps_in[field], irreps_out=self.irreps_out[out_field]
        )

        self.linear_pol = Linear(
            irreps_in='0e', irreps_out=self.irreps_out[out_field]
        )

        self.linear_esp = Linear(
            irreps_in=self.irreps_out[out_field], irreps_out=self.irreps_out[out_field]
        )

        self.tensor_product_layer = FullyConnectedTensorProduct(self.irreps_in[field], '0e', self.irreps_out[out_field])

    def forward(self, data: AtomicDataDict.Type) -> AtomicDataDict.Type:
        # esp = data[AtomicDataDict.ELEC_POTENTIAL_KEY].to('cuda').repeat(1, self.esp_repeat)
        # data[self.out_field] = torch.concat((self.linear(data[self.field]), esp), dim=1)
        esp = data[AtomicDataDict.ELEC_POTENTIAL_KEY]
        esp = esp.to(data[self.field].device)
        # print(esp.device)
        # print(data[self.field].device)
        data[self.out_field] = self.linear(data[self.field]) + self.linear_pol(esp)
        return data
...
class AtomwiseReduce(GraphModuleMixin, torch.nn.Module):
    ...
    def forward(self, data: AtomicDataDict.Type) -> AtomicDataDict.Type:
        ...
        # print('Output Type')
        # print(data[self.out_field].dtype)
