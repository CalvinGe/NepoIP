...
class InteractionBlock_pol(GraphModuleMixin, torch.nn.Module):
    avg_num_neighbors: Optional[float]
    use_sc: bool
    ...
    def __init__(
        ...
        if self.use_sc:
            ...
            self.sc_pol = FullyConnectedTensorProduct(
                feature_irreps_in,
                '1x0e',
                feature_irreps_out,
            )
    ...
    def forward(self, data: AtomicDataDict.Type) -> AtomicDataDict.Type:
        ...
        if self.sc is not None:
            # print('NODE_ATTRS_KEY Type')
            # print(data[AtomicDataDict.NODE_ATTRS_KEY].dtype)
            # print('NODE_ATTRS_KEY Device')
            # print(data[AtomicDataDict.NODE_ATTRS_KEY].device)

            # print('ESP Type')
            # print(data[AtomicDataDict.ELEC_POTENTIAL_KEY].dtype)
            # print('ESP Device')
            # print(data[AtomicDataDict.ELEC_POTENTIAL_KEY].device)

            sc_ori = self.sc(x, data[AtomicDataDict.NODE_ATTRS_KEY])
            sc_pol = self.sc_pol(x, data[AtomicDataDict.ELEC_POTENTIAL_KEY])
            sc = sc_ori + sc_pol
