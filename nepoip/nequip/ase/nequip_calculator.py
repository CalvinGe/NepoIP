...
class NequIPCalculator(Calculator):
    ...
    def calculate_p(self, atomic_numbers, positions, properties=["energy"], system_changes=all_changes):
        """
        Calculate properties.
        ...
        # prepare data
        add_fields={}
        add_fields[AtomicDataDict.ATOMIC_NUMBERS_KEY] = atomic_numbers
        data = AtomicData.from_points(pos=positions, r_max=self.r_max, **add_fields)
        # print(data)
        for k in AtomicDataDict.ALL_ENERGY_KEYS:
            if k in data:
                del data[k]
