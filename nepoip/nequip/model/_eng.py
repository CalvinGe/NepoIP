...
def EnergyModel_pol_0(
    ...
    layers = {
        ...
        "radial_basis": RadialBasisEdgeEncoding,
        # -- Embed features --
        "chemical_embedding": AtomwiseLinear_pol,
    }
...
def EnergyModel_pol(
    ...
    for layer_i in range(num_layers):
        if layer_i == 0:
            layers[f"layer{layer_i}_convnet"] = ConvNetLayer_pol
        else:
            layers[f"layer{layer_i}_convnet"] = ConvNetLayer
...
def EnergyModel_pol_d(
    ...
    layers.update(
        {
            ...
            "output_hidden_to_charge": (
                AtomwiseLinear,
                dict(irreps_out="1x0e", out_field=AtomicDataDict.PER_ATOM_CHARGE_KEY),
            ),
        }
    )
    ...
    layers["total_energy_sum"] = (
        ...
        dict(
            reduce="sum",
            field=AtomicDataDict.PER_ATOM_ENERGY_KEY,
            out_field=AtomicDataDict.SELF_ENERGY_KEY,
        ),
    )

    layers["coupl_energy_sum"] = (
        AtomwiseReduce,
        dict(
            reduce="sum",
            field=AtomicDataDict.PER_ATOM_CHARGE_KEY,
            out_field=AtomicDataDict.COUPL_ENERGY_KEY,
        ),
    )
